...
const initPicker={
	...
	range:{
		init(start,end,value,flag){
			...
			let curMonth=flag?startDVal[1]*1:(startDVal[1]*1+1);
			let curMonth1=flag?endDVal[1]*1:(endDVal[1]*1+1);
			let totalDays=new Date(startDVal[0],startDVal[1],0).getDate();
			let totalDays1=new Date(endDVal[0],endDVal[1],0).getDate();
			for(let s=startYear;s<=endYear;s++){
				fyears.push(s+'');
			};
			for(let m=1;m<=12;m++){
			...
			for(let s=startDVal[0]*1;s<=endYear;s++){
				tyears.push(s+'');
			};
