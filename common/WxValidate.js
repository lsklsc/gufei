...
class WxValidate {
    ...
    __initMethods() {
        ...
        that.methods = {
			...
			chinese(value) {
			    return that.optional(value) || /^[\u0391-\uFFE5]+$/.test(value)
			},
			...
			// 验证车牌号
			carNum(value){
				return that.optional(value) || /^([京津晋冀蒙辽吉黑沪苏浙皖闽赣鲁豫鄂湘粤桂琼渝川贵云藏陕甘青宁新][ABCDEFGHJKLMNPQRSTUVWXY][0-9DF][0-9ABCDEFGHJKLMNPQRSTUVWXYZ]\d{3}[1-9DF]|[京津晋冀蒙辽吉黑沪苏浙皖闽赣鲁豫鄂湘粤桂琼渝川贵云藏陕甘青宁新][ABCDEFGHJKLMNPQRSTUVWXY][\dABCDEFGHJKLNMxPQRSTUVWXYZ]{5})$/.test(value)
			},
            /**
             * 验证日期格式
             */
